...
export async function getProjectChunks(projectId) {
  try {
    ...
    try {
      ...
      console.log('tocFilestocFilestocFilestocFiles', tocFiles)
    ...
    console.log('tocByFile', tocByFile, '')
    let tocs = "";
    if (files.length > 0)
      tocs = markdownSplitter.tocToMarkdown(tocByFile['医学数据集' + ".md"], { isNested: true })
    // 整合结果
    let fileResult = {
